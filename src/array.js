// Initialize an empty array to hold the todo items
const todoArray = []
...
document.querySelector('#submit').addEventListener('click', () => {
    ...
    // Check if inputData is not empty before proceeding
    if (inputData.trim() === '') {
    ...
    // Add the inputData to the todoArray
    addToTodoArray(inputData)
    ...
    todo_delete_el.addEventListener('click', (e) => {
        ...
        removeFromTodoArray(inputData)
    })
})

// Function to add a todo item to the array
function addToTodoArray(todoItem) {
    todoArray.push(todoItem)
    console.log('Todo Array:', todoArray)
}

// Function to remove a todo item from the array
function removeFromTodoArray(todoItem) {
    const index = todoArray.indexOf(todoItem)
    if (index > -1) {
        todoArray.splice(index, 1)
    }
    console.log('Todo Array after removal:', todoArray)
}
